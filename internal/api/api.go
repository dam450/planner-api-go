...
func (api *API) PatchParticipantsParticipantIDConfirm(w http.ResponseWriter, r *http.Request, participantID string) *spec.Response {
	...
	if err != nil {
		return spec.PatchParticipantsParticipantIDConfirmJSON400Response(spec.BadRequestError{Message: "uuid is not valid"})
	}
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return spec.PatchParticipantsParticipantIDConfirmJSON400Response(spec.BadRequestError{Message: "participant not found"})
		}
		...
		return spec.PatchParticipantsParticipantIDConfirmJSON400Response(
			spec.BadRequestError{Message: "something went wrong, try again"},
		)
	}

	if participant.IsConfirmed {
		return spec.PatchParticipantsParticipantIDConfirmJSON400Response(
			spec.BadRequestError{Message: "participant already confirmed"})
	}
